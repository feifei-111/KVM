...
KVM_TEST(graph_round_trip_with_tile_ops) {
  ...
  ValueNode* x = root->AddArgument(
      Value{"x", T(), F16({8, 16}, Location::kShared, {0, 8})});
  ValueNode* w =
      root->AddArgument(Value{"w", T(), F16({16, 16}, Location::kShared)});

  Operator mma = *reg.GetOperator("tile.tc.mma");
  OpNode* mm =
      root->MakeOperation(Operation{mma, MmaOp{8, 16, 16}}, {x, w},
...
KVM_TEST(verify_mma_rejects_shape_mismatch) {
  ...
  auto reg = MakeRegistry();
  Graph g;
  Block* root = g.MakeRoot("e");
  ValueNode* a =
      root->AddArgument(Value{"a", T(), F16({8, 16}, Location::kShared)});
  Operator mma = *reg.GetOperator("tile.tc.mma");

  // inner dims disagree (A.k=16, B.k=8) -> fails before the legality TODO
  ValueNode* b2 =
      root->AddArgument(Value{"b2", T(), F16({8, 32}, Location::kShared)});
  OpNode* bad = root->MakeOperation(
      Operation{mma, MmaOp{8, 32, 16}}, {a, b2},
      {Value{"d2", T(), F16({8, 32}, Location::kRegister)}});
  KVM_CHECK(!Verify(bad).ok);
}
...
KVM_TEST(verify_mma_rejects_bad_accumulator_loc) {
  ...
  ValueNode* a =
      root->AddArgument(Value{"a", T(), F16({8, 16}, Location::kShared)});
  ValueNode* b =
      root->AddArgument(Value{"b", T(), F16({16, 32}, Location::kShared)});
  Operator mma = *reg.GetOperator("tile.tc.mma");
  // D in shared (must be register) -> illegal
  OpNode* bad =
      root->MakeOperation(Operation{mma, MmaOp{8, 32, 16}}, {a, b},
...
KVM_TEST(verify_move_direction) {
  ...
  Block* root = g.MakeRoot("e");
  ValueNode* s =
      root->AddArgument(Value{"s", T(), F16({8, 16}, Location::kShared)});
  Operator ld = *reg.GetOperator("tile.lsu.smem_to_reg");
  // shared -> register : legal
  OpNode* ok =
      root->MakeOperation(Operation{ld, {}}, {s},
                          {Value{"r", T(), F16({8, 16}, Location::kRegister)}});
  KVM_CHECK(Verify(ok).ok);
  // same op but output in shared : wrong dst loc
  OpNode* bad =
      root->MakeOperation(Operation{ld, {}}, {s},
                          {Value{"r2", T(), F16({8, 16}, Location::kShared)}});
  KVM_CHECK(!Verify(bad).ok);
}
...
KVM_TEST(verify_elementwise_shape_mismatch) {
  ...
  Block* root = g.MakeRoot("e");
  ValueNode* x =
      root->AddArgument(Value{"x", T(), F16({8, 16}, Location::kRegister)});
  ValueNode* y =
      root->AddArgument(Value{"y", T(), F16({8, 8}, Location::kRegister)});
  Operator add = *reg.GetOperator("tile.alu.add");
  OpNode* bad =
      root->MakeOperation(Operation{add, {}}, {x, y},
